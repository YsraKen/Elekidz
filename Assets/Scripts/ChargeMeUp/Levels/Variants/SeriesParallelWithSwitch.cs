...
namespace ChargeMeUp
{
	...
	// "2 bulbs light up pag flinip/Turn ON ung switch while the one next to the battery is always ON"
	// "walang wire na walang connection (open circuit)"

	public class SeriesParallelWithSwitch : Level
	{
...
#region Search for Switch
			...
			bool isSeriesLoopHasSwitch = default;
			
			foreach(var element in seriesLoop.elements)
			{
				var component = element as Component;
				if(!component) continue;
				
				isSeriesLoopHasSwitch = component.ComponentId == _switchId;
				if(isSeriesLoopHasSwitch) break;
			}
			
			if(!isSeriesLoopHasSwitch)
			{
